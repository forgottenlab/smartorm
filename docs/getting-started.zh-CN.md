...
本文描述当前 2.0.x 的源码/本地模块接入方式。SmartORM 尚未提供已发布的 Spring Boot Starter，也没有经过验证的 Maven Central 公共制品。
...
评估时请把本仓库作为 Maven 项目打开，或作为本地源码模块引入。项目坐标是 `io.github.forgottenlab:smartorm:2.0.0`，但尚未验证公共仓库可用性。

当前单体 artifact 还包含 demo 代码，以及直接的 MPJ、MySQL 和 Web 依赖。接入其他项目之前请先阅读[兼容性](compatibility.zh-CN.md)。
