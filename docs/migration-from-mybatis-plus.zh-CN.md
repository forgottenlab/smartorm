...
SmartORM 是增强而不是替代。迁移应当渐进、由测试驱动且可以回滚。现有 `BaseMapper`、Wrapper、XML、Provider、MyBatis-Plus 注解和 MyBatis-Plus-Join 实现继续有效。

## 可用性总览

| Level | 目标 | 当前 2.0.x 状态 |
|---|---|---|
| 0 | 添加当前源码/本地模块但不迁移方法 | 可用于评估；纯公共依赖接入尚未验证 |
| 1 | 接入一个 Mapper | 已实现 |
| 2 | 接入一个方法 | 已实现 |
...
- 添加本地模块前后都运行原应用及测试套件。
- 记录当前依赖面：2.0.x 仍是单模块 artifact，包含 demo、MPJ、MySQL 和 Web 依赖。

当前限制：没有经过验证的 Maven Central 制品或 Starter，因此“只添加依赖”目前是兼容性目标，不是已经验证的公开安装体验。
