...
| GitHub Actions workflow definition | ✅ Complete | JDK 17, core 28, Starter 8, MySQL 46, package, report upload, always-cleanup |
...
| Starter feature workflow result | ⏳ Pending | Requires a separately authorized feature push and exact-SHA run |
...
| Starter build/feature/docs checkpoints | 🧪 Locally Verified | Explicit-path local commits; boundaries checked before and after each commit |
| Starter feature remote validation | ⏳ Pending | Feature branch is intentionally not pushed in the foundation task |
...
| Feature push, tag, GitHub Release | ⏳ Pending | Require separate explicit authorization |

## 📋 Current Decision

`STARTER_FOUNDATION_LOCALLY_VERIFIED_REMOTE_PENDING`.

The 2.0.x foundation main SHA `c6e8c8b` passed its exact remote workflow. The 2.1.x Starter foundation has passed local context, consumer, core, MySQL, package, artifact, dependency, and cleanup gates, but its feature checkpoint has not been pushed or remotely verified. Signing/repository setup, Maven Central publication, tag, and release remain pending; no bit-for-bit reproducibility claim is made.
